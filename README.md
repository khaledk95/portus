...
- **One-click SSM shell** — opens `aws ssm start-session` in a new terminal window. On Windows that is PowerShell, for its tab completion; macOS uses Terminal; Linux uses the first of `gnome-terminal`, `konsole`, `xterm` or `x-terminal-emulator` found on `PATH`
